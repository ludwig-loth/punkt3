...
const { data: imprintData } = await useAsyncData('imprint', () => {
  return $directus.request($readItems('imprint', {
    fields: ['*',
      'translations.*',
    ],
  }))
...
<template>
  <div class="flex flex-col w-full gap-3 post-content">
    <div class="p-1 rounded-sm bg-base-100">
      <div class="border-2 border-dotted rounded-sm bg-base-100 border-base-200">
        <h2 class="p-2 pb-0">{{ tStatic('imprint') }}</h2>
        <div class="px-2 pb-2">
          <h3 id="m46">{{ tStatic('service_provider') }}</h3>
          <div>{{ imprintData.full_name }}</div>
          <div>{{ imprintData.street_and_number }}</div>
          <div>{{ imprintData.zipcode_and_town }}</div>
          <div>{{ imprintData.country }}</div>
          <div class="flex flex-row items-start gap-2 p-2 pl-0 text-xs text-accent-content">
            ...
            <div>{{ t(imprintData, 'address_info') }}</div>
          </div>
          <h3 id="m56">{{ tStatic('contact') }}</h3>
          <div>E-Mail: <span>{{ imprintData.mail_address }}</span>
          </div>
          <div>{{ tStatic('contact_form') }}: <NuxtLink to="/contact">
              {{ imprintData.contact_form_url }}
            </NuxtLink>
          </div>
          <div v-html="t(imprintData, 'text')" class="pt-6"></div>
        </div>
      </div>
    </div>
  </div>
