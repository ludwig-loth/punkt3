...
const isActiveOrChild = (menuItemPath) => {
  return route.fullPath === menuItemPath || route.fullPath.endsWith('/' + menuItemPath);
};

const getParentRoute = () => {
  const pathSegments = route.fullPath.split('/').filter(Boolean); // Split the path into segments
  pathSegments.pop(); // Remove the last segment (current route)
  return `/${pathSegments.join('/')}` || '/'; // Reconstruct the parent path or fallback to root
};
...
const navigationLinks = computed(() => {
  ...
  const prev = currentIndex > 0 ? items[currentIndex - 1].slug : null;
  const next = currentIndex < items.length - 1 ? items[currentIndex + 1].slug : null;
