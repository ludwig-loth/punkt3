...
// const { data } = storeToRefs(languageStore)
...
async function sendContactForm() {
  try {
    ...
    const result = await $directus.request($createItem('contact_form', contactFrom.value));
...
<template>
  <div v-if="contactStore.data && contactStore.data.socials"
    class="flex flex-col w-full gap-3 post-content">
    <div class="p-1 rounded-sm bg-base-100">
      <div class="border-2 border-dotted rounded-sm bg-base-100 border-base-200">
        ...
        <div class="px-2 pb-2" v-html="t(contactStore.data, 'contact_introduction')"></div>
        <div
          class="flex flex-row flex-wrap w-fit *:flex *:flex-row *:items-center *:justify-start p-2 pb-4 gap-5 relative justify-center">
          <contact-card v-for="social in cvStore.data.socials" :item="social.contact_socials_id"
            class="post-content-none"></contact-card>
        </div>
      </div>
    </div>
    <div v-if="contactStore.data.show_direct_message" class="p-1 rounded-sm bg-base-100">
      <div class="border-2 border-dotted rounded-sm bg-base-100 border-base-200">
        <h2 class="p-2 pb-0">{{ tStatic('direct_message') }}</h2>
        <div class="p-2">
          <div v-html="t(contactStore.data, 'direct_message_introduction')"></div>
          <div>
            <form @submit.prevent="sendContactForm"
              ...
              <div class="flex flex-col gap-1 group/1">
                <label
                  ...
                  for="name">Name</label>
                <input id="name" type="text" v-model="contactFrom.name" :placeholder="tStatic('name_placeholder')"
              ...
              <div class="flex flex-col gap-1 group/2">
                ...
                <input id="mail" type="email" v-model="contactFrom.mail" :placeholder="tStatic('mail_placeholder')"
              ...
              <div class="flex flex-col gap-1 group/3">
                ...
                <textarea id="text" ref="textarea" v-model="input" :placeholder="tStatic('message_placeholder')"
                  rows="5"
          ...
          <div class="flex flex-row items-start gap-2 p-2 text-xs text-accent-content">
            ...
            <div v-html="t(contactStore.data, 'gdpr_info')"></div>
          </div>
        </div>
      </div>
    </div>
